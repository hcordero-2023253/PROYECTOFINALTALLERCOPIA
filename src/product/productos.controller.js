...
export const addProduct = async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).send({ success: false, message: 'Cannot add product', error });
    }
}
...
export const updateProduct = async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).send({ success: false, message: 'Cannot update product', error });
    }
}
...
export const deleteProduct = async (req, res) => {
    try {
        ...
        if (!product) {
            return res.status(404).send({ success: false, message: 'The product does not exist' });
        }
        ...
        console.error(error);
        res.status(500).send({ success: false, message: 'Cannot delete product', error });
    }
};
